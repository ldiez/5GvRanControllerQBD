...
void LogManager::Register(LogModule &logModule)
{
...
void LogManager::SetLogLevel(std::string moduleName, LogLevel level)
{
...
void LogModule::CheckEnvVar(void)
{
  ...
  std::size_t token_ = logVar_.substr(beg_, logVar_.size() - beg_).find(":");

  logVar_ = token_ == std::string::npos ? logVar_.substr(beg_, logVar_.size() - beg_) : logVar_.substr(beg_, token_ - beg_);
